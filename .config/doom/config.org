#+TITLE: YSB's Doom Emacs Config
#+AUTHOR: Derek Taylor (DT) and Yuvraj Birdi (YSB)
#+DESCRIPTION: DT's personal  Doom Emacs config modified by YSB.
...
#     :desc "Insert jupyter src block"  "s" #'my/insert-jupyter-src-block
